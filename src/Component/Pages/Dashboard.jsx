...
const Dashboard = () => {
  const [registerDriver, setRegisterDriver] = useState(0);
  const [driverOnCurrentRide, setDriverONCurrentRide] = useState(0);
  const [activeDriver, setActiveDriver] = useState(0);
  const [activeRide, setActiveRide] = useState(0);
  const [totalEarnings, setTotalEarnings] = useState(0);
  const [yourEarning, setYourEarnings] = useState(0);
  ...
  useEffect(() => {
    ...
    const fetchYourEarnings = async () => {
      const apiUrl = `${BACKEND_URI}/eto/api/v1/admin/totalEarning/683c1f9d95518b0e2d0e71d0`;
      try {
        const response = await axios.get(apiUrl);
        setYourEarnings(response.data.data.totalEarnings || 0);
      } catch (error) {
  ...
  <CardDataStats 
    ...
    total={formatNumber(activeDriver)} 
    rate=""
    totalClassName="text-amber-600 dark:text-amber-400"
  >
    <svg
      className="fill-amber-600 dark:fill-white"
      width="22"
      height="16"
      viewBox="0 0 22 16"
      fill="none"
  ...
  <CardDataStats 
    ...
    total={`₹${formatNumber(totalEarnings)}`} 
    rate=""
  ...
  <CardDataStats 
    title="Your Earnings" 
    total={`₹${formatNumber(yourEarning)}`} 
    rate=""
    totalClassName="text-purple-600 dark:text-purple-400"
  >
    <svg
      className="fill-purple-600 dark:fill-white"
      width="22"
      height="18"
      viewBox="0 0 22 18"
      fill="none"
