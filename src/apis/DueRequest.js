...
export const dueRequestAPI = {
  ...
  getPendingDueRequests: async () => {
    try {
      ...
      } else if (response.data.data && Array.isArray(response.data.data.requests)) {
  ...
  getDueRequestById: async (requestId) => {
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        requestsData = findArrayInObject(response.data) || [];
      }
      
      const foundRequest = requestsData.find(req => req._id === requestId);
      return foundRequest;
    } catch (error) {
